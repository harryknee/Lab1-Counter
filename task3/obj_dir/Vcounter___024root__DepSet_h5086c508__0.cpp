...
void Vcounter___024root___eval_debug_assertions(Vcounter___024root* vlSelf) {
    ...
    if (VL_UNLIKELY((vlSelf->storedVal & 0xfeU))) {
        Verilated::overWidthError("storedVal");}
    if (VL_UNLIKELY((vlSelf->lastEn & 0xfeU))) {
        Verilated::overWidthError("lastEn");}
}
#endif  // VL_DEBUG
